...
def load_args():
    ...
    parser.add_argument('--nd', default=3, type=int)
...
def eval_nn_f(data, layers):
    ...
    h1_w, h1_b = h1
    h2_w, h2_b = h2
    x = torch.cos(F.linear(data, h1_w, bias=h1_b))
...
def plot_data_entropy(x, y, ents, title):
    ...
    if not os.path.exists(args.save_dir+'/pickle'):
        os.makedirs(args.save_dir+'/pickle')
    #with open('{}/pickle/{}.pickle'.format(args.save_dir,title), 'w') as f:
    pickle.dump(fig, file('this.pickle'))
...
def get_points(netE, W1, W2, iter, net=None):
    ...
    for x1 in np.linspace(-20, 30, 5):
        for x2 in np.linspace(-20, 35, 5):
            for x3 in np.linspace(-25, 35, 5):
                ...
                for (h1_w, h1_b, h2_w, h2_b) in zip(l1_w, l1_b, l2_w, l2_b):
                    data = torch.tensor([x1, x2, x3]).cuda()
                    if net is not None:
                        x = net(data)
                    else:
...
def create_data(args):
    dist1 = MultivariateNormal(torch.tensor([4.2, 6.2, 5.2]), torch.eye(3)*.09)
    dist2 = MultivariateNormal(torch.tensor([4.8, 5.2, 5.]), torch.eye(3)*0.06)
    dist3 = MultivariateNormal(torch.tensor([6., 6.0, 6.]), torch.eye(3)*.09)
    dist4 = MultivariateNormal(torch.tensor([6., 3.94, 4.95]), torch.eye(3)*0.08)
    p1 = dist1.sample((25,))
    p2 = dist2.sample((25,))
    p3 = dist3.sample((25,))
    p4 = dist4.sample((25,))
    x = torch.stack([p1, p2, p3, p4]).view(-1, args.nd).cuda()
